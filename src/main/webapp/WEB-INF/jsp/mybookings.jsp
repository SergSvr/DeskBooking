...
<head>
    ...
    <script>
        ...
        $(function () {
            $("#bookingDate").datepicker().datepicker("option", $.datepicker.regional["ru"]);
            $("#startTime").timepicker({'scrollDefault': 'now', 'timeFormat': 'H:i'});
            $("#endTime").timepicker({'scrollDefault': 'now', 'timeFormat': 'H:i'});
        });
    </script>
</head>
...
<main role="main" class="container">
    ...
    <div align="center">
        <table border="1" cellpadding="5" class="table">
            ...
            <c:forEach var="i" items="${bookings}">
                <tr>
                    ...
                    <td><a href="/mybookings/delete?id=${i.id}">Cancel Booking</a></td>
                </tr>
            </c:forEach>
        </table>
        </c:if>
        ...
        <form class="form" method="post" action="/mybookings/find">
            <input type="text" value="${date}" id="bookingDate" name="bookingDate" required>
            <input type="text" id="startTime" name="startTime" placeholder="timeFrom" required autofocus>
            <input type="text" id="endTime" name="endTime" placeholder="timeTo" required autofocus>
            <button class="btn" type="submit">Find Booking</button>
        </form>
        <c:out value="${result}"/>
        <c:out value="${error}"/>
    </div>
